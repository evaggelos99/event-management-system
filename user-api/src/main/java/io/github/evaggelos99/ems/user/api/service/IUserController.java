...
public interface IUserController extends IGenericController {
    ...
    @PostMapping("/{entityUuid}")
    Mono<UserDto> postUser(@Valid @RequestBody UserDto userDto, @PathVariable UUID entityUuid);
     ...
     * Method that gets a User object from the DB
     *
     * @param entityId the UUID that will be used to search for the entity returning the User
     */
    @Operation(summary = "GET operation that returns a user object", description = "retrieves a User object from the data source")
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "successful operation"),
            @ApiResponse(responseCode = "404", description = "could not find the object")})
    ...
    Mono<UserDto> getEntityUser(@PathVariable UUID entityId);
    ...
    Mono<ResponseEntity<Void>> deleteUser(@PathVariable UUID userId);
